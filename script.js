document.addEventListener('DOMContentLoaded', () => {
    ...
    function addMonthlyContributionInput(month = null, amount = null) {
        ...
        monthlyContributionInputs.appendChild(con579tributionDiv);
    }
    ...
    minhasMetasBtn.addEventListener('click', () => {
        ...
        metasSalvas.forEach((meta, index) => {
            ...
            const monthsCount = meta.investmentPeriod || 12;
            ...
            metaItem.innerHTML = `
                ...
                <div class="meta-months-list">
                    ${monthsList.map(monthNum => {
                        const monthData = (meta.detalhamentoMensal || [])[monthNum - 1] || {};
                        
                        return `
                            <div class="meta-month-item">
                                <div class="meta-month-header">
                                    <input type="checkbox" class="month-checkbox" data-month="${monthNum}">
                                    <strong>${monthNum}</strong>
                                </div>
                                <div class="meta-month-details">
                                    <p>Contribuição: ${monthData.contribuicao || 'R$ 0,00'}</p>
                                    <p>Investimento Acumulado: ${monthData.investimentoAcumulado || 'R$ 0,00'}</p>
                                    <p>Rendimento: ${monthData.rendimento || 'R$ 0,00'}</p>
                                </div>
                            </div>
                        `;
                    }).join('')}
            ...
            function updateMetaTotal() {
                const checkedCheckboxes = Array.from(monthCheckboxes).filter(checkbox => checkbox.checked);

                let totalValue = 0;
                checkedCheckboxes.forEach(checkbox => {
                    const monthItem = checkbox.closest('.meta-month-item');
                    const accumulatedValueText = monthItem.querySelector('p:nth-child(2)').textContent;
                ...
                const metaState = Array.from(monthCheckboxes).map(checkbox => ({
